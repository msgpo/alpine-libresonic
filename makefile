...
MOUNTFLAGS := -v $(CURDIR)/config:/config # -v $(CURDIR)/music:/music
...
test :
	...
	if [ "$(HOSTARCH)" = "armhf" ] || [ "$(ARCH)" != "armhf"  ]; then \
	    docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) sh -ec 'sleep 25; java -version'; \
	fi;
